...
- Same input = same output, no side effects, no mutation of external state
...
- Use language-specific mechanisms (try/catch, Result, error returns)

### 3. Input Validation
**ALWAYS** validate input data:
- Check for null/nil/None values, validate types, ranges, and constraints
- Sanitize user input, return clear validation error messages

### 4. Security
**NEVER** expose sensitive information:
- Don't log passwords, tokens, or API keys
- Use environment variables for secrets
- Sanitize all user input, use parameterized queries (prevent SQL injection)
...
**ALWAYS** write tests:
- Unit tests for pure functions, integration tests for components
...
- Include examples, keep it up to date, use consistent formatting
