<!-- Context: openagents-repo/context-bundle-template | Priority: low | Version: 1.1 | Updated: 2026-08-08 -->
...
## Relevant Standards (Load Before Starting)
...
**From Standards**:
- {requirement 1 from code-quality.md}
- {requirement 2 from test-coverage.md}
- {requirement 3 from documentation.md}
...
**IMPORTANT**:
1. Load ALL context files listed before starting
...
**Questions/Clarifications**: {Any questions the subagent should consider}
```
...
1. **Create session directory**: `mkdir -p .tmp/context/{session-id}`
2. **Copy template**: `cp .opencode/context/openagents-repo/templates/context-bundle-template.md .tmp/context/{session-id}/bundle.md`
3. **Fill in all sections**: Replace `{placeholders}`, list specific context files, define clear success criteria, provide specific instructions
4. **Pass to subagent**:
```javascript
...
**DO**: ✅ List context files with full paths (don't duplicate content) | ✅ Extract key requirements | ✅ Define binary success criteria (pass/fail) | ✅ Provide specific validation requirements | ✅ Include clear instructions for subagent

**DON'T**: ❌ Duplicate full context file content (just reference paths) | ❌ Use vague success criteria | ❌ Skip validation requirements | ❌ Forget to list technical constraints
